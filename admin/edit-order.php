<?php
// admin/edit-order.php - עריכת הזמנה על ידי מנהל
...
if ($_POST) {
    ...
    // עדכון על ידי מנהל - מתודה מיוחדת
    $result = $order->updateOrderByAdmin($id, $updateData, $currentUser['id']);
    
    if ($result['success']) {
        // שליחת התראה ללקוח
        $notification->notifyOrderUpdatedByAdmin($id, $orderDetails['customer_id'], $currentUser['id']);
        
        $message = 'ההזמנה עודכנה בהצלחה והלקוח קיבל התראה על השינוי';
        // רענון נתוני ההזמנה
        $orderDetails = $order->getOrderById($id);
    } else {
...
<head>
    ...
    <title>עריכת הזמנה - <?php echo SITE_NAME; ?></title>
...
<body>
    ...
    <div class="container" style="margin-top: 2rem;">
        <div class="d-flex justify-between align-center mb-3">
            <h1>עריכת הזמנה #<?php echo $orderDetails['order_number']; ?></h1>
            <a href="orders.php?action=view&id=<?php echo $id; ?>" class="btn btn-outline">חזרה לצפייה</a>
        </div>
        ...
        <div class="alert alert-warning">
            <strong>שים לב:</strong> אתה עורך הזמנה של הלקוח 
            <strong><?php echo htmlspecialchars($orderDetails['first_name'] . ' ' . $orderDetails['last_name']); ?></strong>
            (<?php echo htmlspecialchars($orderDetails['email']); ?>).
            הלקוח יקבל התראה על השינויים שתבצע.
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="editOrderForm">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" readonly 
                                       value="<?php echo htmlspecialchars($orderDetails['first_name'] . ' ' . $orderDetails['last_name']); ?>">
                            </div>
                        </div>
                    </div>
            ...
            <div style="text-align: center; margin-top: 2rem;">
                <button type="submit" class="btn btn-primary" style="padding: 1rem 2rem;" onclick="return confirm('האם אתה בטוח שברצונך לעדכן את ההזמנה? הלקוח יקבל התראה על השינוי.')">
                    עדכן הזמנה ושלח התראה ללקוח
                </button>
                <a href="orders.php?action=view&id=<?php echo $id; ?>" class="btn btn-secondary" style="padding: 1rem 2rem;">
                    ביטול
                </a>
            </div>
        </form>
